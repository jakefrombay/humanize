...
Another useful design pattern is the perception–decision–action loop. In this structure, the agent continuously cycles through three stages: observing the environment, deciding what to do, and performing an action. During the perception stage, the agent collects information from inputs such as user queries, APIs, or sensors. In the decision stage, the agent evaluates this information using reasoning models, policies, or rules to determine the best action. In the action stage, the agent executes the decision, which could involve generating a response, calling a tool, or updating memory. This pattern allows agents to operate in dynamic environments and adapt to new information over time.

The tool-augmented agent pattern focuses on extending the agent’s abilities by connecting it to external tools and services. Instead of relying only on its internal knowledge, the agent can call APIs, databases, search engines, or code execution environments when necessary. The agent decides when a tool is needed, formulates the request, processes the returned data, and integrates it into its final output. This design significantly increases the practical capabilities of the agent and allows it to solve real-world problems that require up-to-date information or computation.

Another pattern is the multi-agent collaboration model, where several autonomous agents work together to solve a larger task. Each agent has its own role, knowledge, or expertise, and they communicate through shared messages or coordination protocols. Some agents may focus on planning, others on verification, and others on execution. Through iterative communication and feedback, the agents gradually converge on a solution. This pattern is useful in research assistants, complex simulations, and distributed AI systems where cooperation among multiple intelligent components leads to better outcomes than a single agent working alone.
"""
INPUT_TEXTS = [p.strip() for p in INPUT_TEXT.strip().split('\n\n') if p.strip()]
...
def main():
    ...
    for i, para in enumerate(INPUT_TEXTS, start=1):
        #print(f"[{i:2d}/10] temperature={temp:.1f} — calling LLM...", end=" ", flush=True)
        try:
            humanized = call_llm(para)
            status = "OK"
    ...
    output_path = os.path.join(os.path.dirname(__file__), "output_openaif.txt")
    with open(output_path, "w", encoding="utf-8") as f:
        ...
        for r in results:
            ...
            f.write(f"temperature: 1.0 (default)\n")
